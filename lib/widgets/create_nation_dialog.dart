...
class _CreateNationDialogState extends State<CreateNationDialog> {
  ...
  final TextEditingController _governmentTypeController = TextEditingController();
  final TextEditingController _eraController = TextEditingController();
  bool _isAC = false;
  bool _isAdvanced = false; // Control para versión simple/avanzada
  ...
  // Economía
  final TextEditingController _economicSystemController = TextEditingController();
  ...
  double _populationGrowth = 1.0; // Valor inicial de crecimiento en porcentaje
  ...
  // Método para crear un slider estilizado
  Widget _buildSlider(String label, double value, Function(double) onChange, {List<String>? sections}) {
    return Column(
      ...
      children: [
        ...
        Slider(
          ...
          label: sections == null ? value.toStringAsFixed(1) : sections[(value ~/ 25).toInt()],
        ...
        if (sections != null)
          Row(
            ...
            children: sections.map((section) => Text(
              section,
  ...
  Widget _buildSectionTitle(String title) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            child: Divider(color: Colors.white.withOpacity(0.5), thickness: 1.0),
          ),
          Padding(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Switch para cambiar entre simple y avanzado
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        fontWeight: _isAdvanced ? FontWeight.normal : FontWeight.bold,
                      ),
                    ),
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        fontWeight: _isAdvanced ? FontWeight.bold : FontWeight.normal,
                      ),
                    ),
                  ],
                ),
                ...
                // Secciones avanzadas (visibles solo si _isAdvanced es true)
                if (_isAdvanced) ...[
                  // Sección de Política
                  ...
                  _buildSlider(
                    ...
                    sections: ['Inestable', 'Frágil', 'Estable', 'Muy Estable', 'Sólida'],
                  ),
                  
                  // Sección de Economía
                  _buildSectionTitle('Economía'),
                  const SizedBox(height: 10),
                  _buildTextField(_economicSystemController, 'Sistema Económico'),
                  const SizedBox(height: 15),
                  _buildTextField(_currencyNameController, 'Nombre de la Moneda'),
                  const SizedBox(height: 15),
                  _buildSlider(
                    ...
                    sections: ['Concentrada', 'Desigual', 'Mixta', 'Equitativa', 'Igualitaria'],
                  ),
                  
                  // Sección de Demografía
                  _buildSectionTitle('Demografía'),
                  const SizedBox(height: 10),
                  _buildTextField(_lifeExpectancyController, 'Expectativa de Vida'),
                  const SizedBox(height: 15),
                  _buildSlider(
                  ...
                  Text(
                    _populationGrowth <= 0 ? 'Decrecimiento poblacional' :
                    _populationGrowth < 1 ? 'Crecimiento lento' :
                    _populationGrowth < 2 ? 'Crecimiento moderado' :
                    _populationGrowth < 3 ? 'Crecimiento rápido' :
                    _populationGrowth < 4 ? 'Crecimiento muy rápido' : 'Auge poblacional',
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    if (_nameController.text.isEmpty ||
                      ...
                      // Crear la nación básica
                      final Map<String, dynamic> nationData = {
                        'name': _nameController.text,
                        'governmentType': _governmentTypeController.text,
                        'era': '${_eraController.text} ${_isAC ? 'a.C' : 'd.C'}',
                      };
                      
                      // Añadir datos avanzados si están habilitados
                      if (_isAdvanced) {
                        nationData.addAll({
                          'leaderName': _leaderNameController.text,
                          'politicalStability': _politicalStability,
                          'economicSystem': _economicSystemController.text,
                          'currencyName': _currencyNameController.text,
                          'wealthDistribution': _wealthDistribution,
                          'lifeExpectancy': _lifeExpectancyController.text,
                          'populationGrowth': _populationGrowth,
                        });
                      }

                      // Disparar evento de creación de nación
